...
public class BalloonRedScript : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Player")
        {
            ...
            AudioSource.PlayClipAtPoint(FireworkSound, new Vector3(5, 1, 2));
            // create a new balloon distant from "Player"
            Transform otherTransform = other.gameObject.GetComponent<Transform>();
            var BalloonTransform = Instantiate(Balloon_bluePrefab) as Transform;
